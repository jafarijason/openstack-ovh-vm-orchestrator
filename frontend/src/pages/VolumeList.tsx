...
export const VolumeList: React.FC = () => {
  ...
  const handleCreateVolume = async (e: React.FormEvent) => {
    ...
    if (formData.size < 1) {
      setError("Volume size must be at least 1 GB")
      return
    }
  ...
  const handleDeleteVolume = async (volumeId: string) => {
    try {
      ...
      setDeletingIds(new Set([...deletingIds].filter((id) => id !== volumeId)))
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">Create Volume</h2>
            </div>
            <form onSubmit={handleCreateVolume} className="p-6 space-y-4">
              <div>
      ...
      {volumes.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-600 text-lg">No volumes yet</p>
          <p className="text-gray-500 text-sm mt-1">
            Create a new volume to get started
          </p>
        </div>
      ) : (
          ...
          {volumes.map((volume) => (
            ...
            >
              <div className="p-6">
                ...
                <div className="mb-4">
                  <span
                    className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(volume.status)}`}
                  >
                  ...
                  {volume.attachments && volume.attachments.length > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-900 text-sm">
                        {volume.attachments[0].vm_id}
                      </span>
                    </div>
                  )}
                  {volume.description && (
                    <div className="pt-2">
                      <p className="text-gray-600">{volume.description}</p>
                    </div>
                  )}
                  {volume.created_at && (
                ...
                {deleteConfirm === volume.id && (
                  <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {deletingIds.has(volume.id) ? "Deleting..." : "Delete"}
                      </button>
                    </div>
                  </div>
                )}
